...
async function createRegistries(workdir: string) {
  const services = await glob(workdir + "/**/*.service.ts");
  const controllers = await glob(workdir + "/**/*.controller.ts");
  const workers = await glob(workdir + "/**/*.worker.ts");
